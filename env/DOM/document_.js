document = new HTMLDocument(myVm.memory.$createObj_key);
; (function () {
    let $attribute_map = {
        'location': location
    }
    ...
    for (const key in $attribute_map) {
        if (Object.hasOwnProperty.call($attribute_map, key)) {
            ...
            myVm.safefunction(Object.getOwnPropertyDescriptor(document, key)['get'], `get ${key}`, true);
            myVm.safefunction(Object.getOwnPropertyDescriptor(document, key)['set'], `set ${key}`, true);
        }
    };
